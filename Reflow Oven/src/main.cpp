...
#define CALIBRATE_SECONDS 10
#define CALIBRATE_TEMP 10

#define DEBUG false

#define PREHEAT_TEMP 60  // temperature to preheat around in degrees C
...
void setup() {
    ...
    while (digitalRead(STARTBTN) && digitalRead(PREHEATBTN)) {  // wait until either button is pressed
    ...
    while (digitalRead(STARTBTN))  // preheat, waiting for actual start
    {
        ...
        temperature = thermoCouple.getCelsius();
        tft.setCursor(0, SCREEN_HEIGHT - TEXT_HEIGHT);
        tft.setTextColor(setRelay(true, PREHEAT_TEMP) ? ILI9341_RED : ILI9341_BLUE, 0x000000);
        tft.print("Preheat: ");
        tft.setTextColor(ILI9341_WHITE, 0x000000);
...
void loop() {
    ...
    tft.drawPixel((millis() - startTime) / 1000 * PpSX, zeroY - (temperature - temprange[0]) * PpDY, isSwitchOn ? ILI9341_RED : ILI9341_BLUE);
